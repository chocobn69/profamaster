'''
A library that allows simple access to 74HC595
shift registers on a Raspberry Pi using any digital I/O pins.
'''
...
# you can change them with shiftRegisters method
...
def pinsSetup(**kwargs):
    ...
    if custompins:
        if _SER_pin != serpin \
                or _RCLK_pin != rclkpin \
                or _SRCLK_pin != srclkpin:
...
def startupMode(mode, execute=False):
    ...
    if isinstance(mode, int):
        if mode is HIGH or mode is LOW:
            ...
            raise ValueError("The mode can be only HIGH or LOW "
                             "or Dictionary with specific pins and modes")
